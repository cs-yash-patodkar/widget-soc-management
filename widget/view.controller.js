...
(function () {
  ...
  function socManagement201Ctrl($scope, config, $q, Query, _, playbookService, $filter, currentDateMinusService, $rootScope, socManagementService, ALL_RECORDS_SIZE, $state, $window) {
    ...
    function _init() {
      ...
      $scope.socResult = {};
      $scope.config.customData = {};
      checkForSVGLoad();
      socManagementService.getCustomData().then(function (response) {
        $scope.config.customData = angular.extend(response.data, config.customData)
      })
